...
void tokenize(string s, string &getsalt, string &gethash, string del = " ")
{
	...
	string Salt = "";
	while (count < 3)
	{ // We are tokenizing based on $ sign so we get 3 tokens
		if (count == 1)
		{ // First token is ID
...
bool PasswordCracker(int digitstart, int digitsend, string salt, string actualencrypt, int procid)
{
	...
	for (; i < digitsend - 1; i++)
	{ // 8 nested loops to create password combinations using alphabets upto 8 characters long
							...
							for (int dig3 = 0; dig3 < 26; dig3++)
							{
								...
								for (int dig2 = 0; dig2 < 26; dig2++)
								{
									...
									for (int dig1 = 0; dig1 < 26; dig1++)
									{
										...
										if (strncmp(currencrypt.c_str(), actualencrypt.c_str(), currencrypt.length()) == 0)
										{ // compare encrypoted key
...
int main(int argc, char **argv)
{
	...
	if (myrank == 0)
	{
		...
		{
			while (file >> line)
			{ // get the encrypted password of given user name from shadow file
		...
		if (remainder > 0)
		{ // if max length not equalli divisible then master will search the remaining
	...
	if (myrank > 0)
	{
		...
		if (PasswordCracker(start_search, end_search, salt, actualencrypt, myrank) == false)
		{ // run function for password search
			cout << "\nPROCESS: " << myrank << " -> PASSWORD WAS NOT FOUND\n"
